...
(with-test 
  (defn simple [dices digit]
    (reduce + (filter (fn [dice] (= dice digit)) dices))
    )
...
(with-test
  (defn several-equals [dices num-equals]
    ...
    (if (empty? pairs) 0 (* num-equals (reduce max (map (fn [entry] (first entry)) pairs))))
    )) 
...
(with-test
  (defn house [dices]
    (if (= (set (vals (frequencies dices))) #{2 3}) (reduce + dices) 0)
    )
...
(with-test
  (defn two-pairs [dices]
    (let [pairs (filter (fn [x] (>= (second x) 2)) (frequencies dices))]
       (if (= (count pairs) 2) (* 2 (reduce + (map (fn [entry] (first entry)) pairs))) 0)
    ))
